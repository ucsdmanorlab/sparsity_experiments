...
data_dir = os.path.join(setup_dir,"../../../data/test.zarr")
rep = setup_dir.split('/')[-1]
net, sparsity = setup_dir.split('/')[-2].split('-') # assuming non-disk sparsity
gt_source = os.path.join(net,sparsity,rep)
print(f"loading labels from {gt_source}/seg")
...
def train(
    ...
    unlabelled = gp.ArrayKey('LABELS_MASK')
    ...
    source = gp.ZarrSource(
            ...
            {
                raw: "raw",
                labels: os.path.join(gt_source,"seg"),
                unlabelled: os.path.join(gt_source,"labels_mask"),
            },
            {
    ...
    pipeline += gp.GrowBoundary(labels, only_xy=True)
...
if __name__ == "__main__":
    ...
    config["max_iteration"] = int(sys.argv[1])
    train(**config)
